...
class MixpanelObserver < ActiveRecord::Observer
  ...
  def from_pending_to_paid(payment)
    ...
    track_event(payment.user, 'Engaged with OppoStarter', properties.merge(action: 'contribution confirmed'))
  ...
  def track_project_owner_engagement(user, action)
    track_event(user, 'Project owner engaged with OppoStarter', user_properties(user).merge(action: action))
  end
